...
const roundassetp1 = ({route,navigation}) => {
  ...
  const init = async () => {
    const response = await fetch('http://10.0.3.2/api/assetP1/getRoundassetP1/'+PersonName.id);
  ...
  _renderItem = ({item,index}) => {
    return (
      ...
      <ScrollView>
        <TouchableOpacity
        onPress={() => navigation.navigate('assetp1result',{assetp1ID : item.asset_id,PersonName : PersonName})}
        >
